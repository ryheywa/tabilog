...
function doPost(e) {
  try {
    ...
    // 更新リクエストの場合（ストーリー編集など）
    if (data.action === 'update' && data.id) {
      console.log('Update request for id:', data.id);
      return updateEntry(data.id, data);
    }
...
function updateEntry(id, data) {
  ...
  console.log('Updating entry id:', id, 'with data:', JSON.stringify(data));
  ...
  try {
    ...
    console.log('Row to update:', rowToUpdate, ', Last row:', lastRow);

    if (rowToUpdate < 2 || rowToUpdate > lastRow) {
      console.log('Invalid row number');
      return createJsonResponse({ success: false, error: '無効なIDです' });
    }
    ...
    if (data.highlight !== undefined) {
      const highlightCol = 16; // ハイライト列
      sheet.getRange(rowToUpdate, highlightCol).setValue(data.highlight);
      console.log('Updated highlight for row', rowToUpdate);
    }

    return createJsonResponse({ success: true, message: '更新しました' });
